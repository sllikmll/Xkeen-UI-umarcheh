...
def test_preview_sources_exist_and_expose_smoke_modes():
    ...
    assert '--smoke' in read('desktop/previews/cpp-native/macos/main.mm')
...
def test_preview_ui_variants_are_separate():
    ...
    assert '#import <Cocoa/Cocoa.h>' in read('desktop/previews/cpp-native/macos/main.mm')

def test_preview_manifest_lists_all_three_artifacts():
    text = read('scripts/build_desktop_previews_manifest.py')
    assert 'Unified-UI-Avalonia-Preview-0.1.0-win-x64.zip' in text
    assert 'Unified-UI-WPF-Preview-0.1.0-win-x64.zip' in text
    assert 'Unified-UI-Cpp-Native-Preview-0.1.0-mac-arm64.zip' in text
